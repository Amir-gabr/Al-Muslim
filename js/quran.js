...
// surat.map((surah) => {
//   let surahName = `
//         <option> ${surah.name}</option>
//             `;
//   document.getElementById("surah-select").innerHTML += surahName;
// });

// // =====================
//  function getSurahNumber(surahNumber = 1) {
//   let url = `http://api.alquran.cloud/v1/surah/${surahNumber}/quran-uthmani -`;
//   axios
//     .get(url)
//     .then(function (response) {
//       let allAyahs = response.data.data.ayahs;
//       //===========  Surah content   =========
//       for (let i = 0; i < allAyahs.length; i++) {
//         let ayah = allAyahs[i];
//         let ayahs = `
//         <div class="d-flex justify-content-center align-items-center mb-3">
//     <span class="text-light fs-4 ms-3 py-1 px-2 rounded-5" id="ayah-text">
//         ${ayah.text}
//         </span>
//         <span class="" id="number-of-ayah"> ${ayah.numberInSurah}</span>
//         </div>
//             `;
//         document.getElementById("surah-container").innerHTML += ayahs;
//       }
//     })
...
//   //
//   document.getElementById("surah-name").innerHTML =
//   document.getElementById("surah-select").value;
//   //
//   let surahNumber = "";
//     for (const surah of surat) {
//       if (surah.name == document.getElementById("surah-select").value) {
//         surahNumber = surah.number;
//       }
//     }
//   //
//   getSurahNumber(surahNumber);
// });
...
function getSurahName() {
  ...
  axios
    .get(url_api)
    .then(function (response) {
      let surat = response.data;
      // console.log(surat);
      // ===========  Surah content   =========
      surat.map((surah, index) => {
        let surahName = `
                <option> ${index + 1}:  سُورَةُ ${surah.titleAr}</option>
                    `;
        document.getElementById("surah-select").innerHTML += surahName;
      });
    })
...
// ========== get surah number ============
function getSurahNumber(surahNumber = 1) {
  let url = `https://raw.githubusercontent.com/semarketir/quranjson/master/source/surah/surah_${surahNumber}.json`;
  axios
    .get(url)
    .then(function (response) {
      let allAyaht = response.data.verse;
      // console.log(allAyaht);
      //===========  Surah content   =========

      Object.keys(allAyaht).forEach((key, index) => {
        const value = allAyaht[key];
        let ayaht = `
      <div class="d-flex justify-content-center align-items-center mb-3">
        <span class="text-light fs-4 ms-3 py-1 px-2 rounded-5" id="ayah-text">
         ${value}
     ...
     `;
        document.getElementById("surah-container").innerHTML += ayaht;
      });
    })
    .catch(function (error) {
      console.log(error);
  ...
  axios
    ...
    .then(function (response) {
      ...
      for (let i = 0; i < surah.length; i++) {
        ...
        if (surah[i].titleAr == document.getElementById("surah-select").value.slice(11, 17)) {
          surahNumber = i + 1;
          getSurahNumber(surahNumber);
        }
      }
